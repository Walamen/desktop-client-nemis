...
// government/teacher/page.tsx and already fixed on the Timetable and
// My Classes pages (see their test files). This mock's `staff` collection
// deliberately uses a DIFFERENT id (STAFF_ID) than the signed-in user's id
// (USER_ID) — the test fails if the page ever passes USER_ID to
...
function installBaseMock() {
  (window as unknown as { nemis: unknown }).nemis = {
    ...
    student: { list: vi.fn(async () => ({ items: [], total: 0, limit: 250, offset: 0 })) },
    attendance: { list: vi.fn(async () => []), record: vi.fn(async () => ({ ok: true })) },
    schoolAdmin: {
