...
// government/teacher/page.tsx and already fixed on the Timetable, My
// Classes, Attendance, and Assignments List pages (see their test files).
// This mock's `staff` collection deliberately uses a DIFFERENT id
// (STAFF_ID) than the signed-in user's id (USER_ID) — the test fails if the
// page ever passes USER_ID to teacher.listAssignments again.
...
describe('Teacher assignment form', () => {
  it('loads teaching assignments via the self-scoped staffId, never the raw login id', async () => {
    ...
    // The class dropdown is only populated once teachingAssignments loaded
    // successfully — its option confirms the self-scoped call actually
    // returned data instead of erroring.
